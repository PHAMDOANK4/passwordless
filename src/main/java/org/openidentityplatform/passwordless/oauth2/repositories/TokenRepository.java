...
public interface TokenRepository extends JpaRepository<Token, UUID> {
    ...
    void revokeAllUserTokens(@Param("user") User user, @Param("now") Instant now);
       ...
       @Modifying
       @Query("UPDATE Token t SET t.revoked = true, t.revokedAt = :now WHERE t.sessionId = :sessionId AND t.revoked = false")
       void revokeAllBySessionId(@Param("sessionId") String sessionId, @Param("now") Instant now);
